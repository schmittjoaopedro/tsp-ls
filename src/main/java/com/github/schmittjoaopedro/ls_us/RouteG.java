...
public class RouteG {
    ...
    public static final int NBGRAND = 5;

    public static final int MAXK = 3;

    public static final int MINK = 4;

    public static final int MAXK1 = 15;

    public static final double MAXREAL = 1.7e38;

    public tourne t = new tourne();

    public tourne t2 = new tourne();

    public proxnoeud[] p1 = new proxnoeud[CoordG.MAXN + 1];

    public proxnoeud[] p2 = new proxnoeud[CoordG.MAXN + 1];
    ...
    public boolean numerote_tourne() {
        ...
        int i, j;
        boolean indtrue;
        int en[] = new int[CoordG.MAXN + 1];

        for (i = 0; i <= CoordG.MAXN; i++) en[i] = 0;
        indtrue = true;
        w = t.ptr;
        for (i = 1; i <= CoordG.MAXN; i++) {
            w.rang = i;
            en[w.noeud] = 1;
            w = w.prochain;
            if (w == t.ptr) break;
        }
        for (j = 0; j <= CoordG.MAXN; j++) {
    ...
    //void RouteG::ajoutx(int x, int k1, coord g[CoordG.MAXN+1],const float d[][CoordG.MAXN+1])
                ...
                if ((xj != xi) && (t.noeudinterne[nj] == 1)) {
                    ...
                    for (l = 1; l <= (2 * yk1 + 1); l++) {
                        ...
                        if ((nl == suiv(suivni, g)) && (l <= yk1))
                            deja_vuxi2 = 1; /* TRUE */
                        if ((!(nlvaut0 == 1)) && (t.noeudinterne[nl] == 1)) {
                            xl = ordre(nl, g);
                            precnl = prec(nl, g);
                            suivnl = suiv(nl, g);
                        }
    ...
    //float RouteG::calculcoutt(const float d[][CoordG.MAXN+1])
    double calculcoutt(double d[][]) {
        tourneelem i, j;
    ...
    //void RouteG::oterx (int x, int k1, coord g[CoordG.MAXN+1],const float d[][CoordG.MAXN+1])
