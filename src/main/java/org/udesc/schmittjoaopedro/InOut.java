...
public class InOut {
    ...
    static void population_statistics()
        ...
        double best_so_far = Ants.best_so_far_ant.tour_length;

        if (stat_report != null) {
            printToFile(stat_report, iteration + "\t" + best_so_far + "\t" + pop_mean + "\t" + pop_stddev + "\t" + (pop_stddev / pop_mean)
    ...
    static void init_program(String[] args)
        ...
        if (!quiet_flag) {
            Writer w;
            try {
                temp_buffer = "result\\best." + Tsp.instance.name;
                ...
                temp_buffer = "result\\cmp." + Tsp.instance.name;
                // // TRACE ( System.out.println("%s\n",temp_buffer); )
                comp_report = new File(temp_buffer);
                w = new OutputStreamWriter(new FileOutputStream(temp_buffer), "UTF8");
                writer.put(comp_report.getName(), new BufferedWriter(w));

                temp_buffer = "result\\stat." + Tsp.instance.name;
                // // TRACE ( System.out.println("%s\n",temp_buffer); )
                stat_report = new File(temp_buffer);
                w = new OutputStreamWriter(new FileOutputStream(temp_buffer), "UTF8");
                writer.put(stat_report.getName(), new BufferedWriter(w));
                printToFile(stat_report, "iteration\tbsf\tpop_mean\tpop_stddev\t(pop_stddev / pop_mean)\t" +
                        "branching_factor\t((branching_factor - 1.)*Tsp.n\tavg_distance\t(avg_distance /Tsp.n)");
            } catch (IOException e) {
